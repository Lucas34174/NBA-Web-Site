...
export default function TeamList({ teams, onTeamClick }: props) {
  ...
  useEffect(() => {
    ...
    if (stateFilter !== "all") {
      filtered = filtered.filter(
        (team) => team.city.toLowerCase() === stateFilter.toLowerCase()
      );
    }
  ...
  return (
    <>
      <div className="container-fluid ">
        <h1 className="display-10 text-center fw-bold text-white mb-0">
          🏀 Équipes NBA
        </h1>
      ...
      <div className="m-5">
          ...
          <div className="mb-2 col-11 row bg-black bg-opacity-25 align-items-center border border-secondary rounded-3 p-3">
            ...
            <div className="col border-end border-secondary">
              <span
                className="text-warning fw-bold"
                onClick={() =>
                  setSortBy(sortBy === "year-desc" ? "year" : "year-desc")
                }
              >
                Founded
              </span>
            ...
            <div className="col border-end border-secondary">
              <span
                className="text-warning fw-bold"
                onClick={() => setSortBy("abbreviation")}
              >
                Abbreviation
              </span>
            </div>
        ...
        {filteredTeams.map((team) => (
          ...
          >
            <TeamRow team={team} header={false} onTeamInfoClick={onTeamClick} />
          </motion.div>
        ))}
      </div>
    </>
